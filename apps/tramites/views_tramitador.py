"""
Vistas para tramitadores: visualizar trámites asignados, aprobar/rechazar.
"""
...
class TramitadorDashboardView(LoginRequiredMixin, View):
    """
    Vista del dashboard del tramitador con sus trámites asignados.
    """
    def get(self, request):
        # Validar que el usuario sea tramitador
        if request.user.rol != 'TRAMITADOR':
            messages.error(request, "Solo los tramitadores pueden acceder a esta página.")
        ...
        return render(request, 'tramitador/tramites_asignados.html', context)


class DetalleTramiteTramitadorView(LoginRequiredMixin, View):
    """
    Vista detallada de un trámite para el tramitador.
    ...
    def get(self, request, tramite_id):
        ...
        # Validar que el tramitador esté asignado a este trámite
        try:
            AprobacionTramiteService.validar_tramitador_asignado(tramite, request.user)
        except PermissionDenied as e:
            messages.error(request, str(e))
            return redirect(reverse('tramitador:dashboard'))
        ...
        return render(request, 'tramitador/detalle_tramite.html', context)
...
class AprobarTramiteView(LoginRequiredMixin, View):
    ...
    def post(self, request, tramite_id):
        # Validar que el usuario sea tramitador
        if request.user.rol != 'TRAMITADOR':
            return JsonResponse({'error': 'Solo los tramitadores pueden aprobar trámites.'}, status=403)
        ...
        try:
            ...
            return redirect(reverse('tramitador:detalle-tramite', kwargs={'tramite_id': tramite_id}))
        except PermissionDenied as e:
            messages.error(request, str(e))
            return redirect(reverse('tramitador:dashboard'))
...
class RechazarTramiteView(LoginRequiredMixin, View):
    ...
    def post(self, request, tramite_id):
        # Validar que el usuario sea tramitador
        if request.user.rol != 'TRAMITADOR':
            return JsonResponse({'error': 'Solo los tramitadores pueden rechazar trámites.'}, status=403)
...
class VisualizarPDFTramiteView(LoginRequiredMixin, View):
    ...
    Solo el tramitador asignado puede ver el PDF.
    """
    def get(self, request, tramite_id):
        # Validar que el usuario sea tramitador
        if request.user.rol != 'TRAMITADOR':
        ...
        # Obtener el trámite
        tramite = get_object_or_404(
            Tramite.objects.select_related('tramitador_asignado'),
            id=tramite_id
        )
...
class EstadoTramiteAPIView(LoginRequiredMixin, View):
    ...
    def get(self, request, tramite_id):
        ...
        # Validar asignación
        if tramite.tramitador_asignado != request.user:
            return JsonResponse({'error': 'No autorizado'}, status=403)
        ...
        badge_html = render_to_string('tramitador/partials/estado_badge.html', {'estado': tramite.estado})
        timeline_html = render_to_string('tramitador/partials/historial_timeline.html', {'historial': historial})
...
class HistorialGeneralView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # Obtener historial de todos los trámites asignados al tramitador
        historial = HistorialCambios.objects.filter(
            tramite__tramitador_asignado=request.user
        ...
        return render(request, 'tramitador/historial_general.html', context)
